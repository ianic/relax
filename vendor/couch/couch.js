...
function CouchDB(name) {
  ...
  // Open a document from the database
    ...
    if (req.status == 404)
      return null;                            
    //ianic ovdje sam dodao fukciju za parsiranje datum
    var result = JSON.parse(req.responseText, function(k,v){return ( typeof v == "string" && v.isJSONDate() ) ? Date.fromJSON(v) : v;});
    if (req.status != 200)
      throw result;
    return result;
  }
